...
import { playlisIdState, playlistState } from '../atoms/playlistAtom'
    ...
    <section className="flex-grow h-screen overflow-y-scroll scrollbar-hide">
      <header className="absolute top-5 right-8">
        <div
          ...
          <img className="rounded-full w-10 h-10" src={session?.user.image} alt={`${session?.user.name} Profile`} />
          <h2 >{session?.user.name}</h2>
          <ChevronDownIcon className='h-5 w-5' />
        </div>
      </header>

      <div className={`flex items-end space-x-7 bg-gradient-to-b to-black ${color} h-80 text-white p-8`}>
